...
export default function Page() {
    ...
    const handleSubmit = useCallback(async () => {
        ...
        if (inputMode === 'video') {
            if (!mediaFile || !patientId || !consultationDate) {
                setError('Please fill in all fields and select a media file.');
                return;
            }
        } else {
        ...
        try {
            ...
            if (inputMode === 'video' && mediaFile) {
                if (includeMSE) {
                    result = await generateTranscriptWithMSE(mediaFile, patientId, consultationDate, onProgress);
                } else {
                    result = await generateTranscript(mediaFile, patientId, consultationDate, onProgress);
                }
            } else {
    ...
    }, [inputMode, mediaFile, textTranscript, patientId, consultationDate, includeMSE]);

    const isFormValid = inputMode === 'video'
        ? (mediaFile && patientId.trim() && consultationDate)
            ...
            <div className="w-full max-w-3xl mx-auto">
                <header className="text-center mb-6">
                    ...
                    <p className="mt-1 text-xs text-slate-400">
                        {process.env.NEXT_PUBLIC_GEMINI_API_KEY ? '🔓 Demo Mode (Client-side)' : '🔒 Production Mode (Server-side)'}
                    </p>
                </header>
                                    ...
                                    <>
                                        ...
                                        <div className="mt-4 flex items-start gap-3 p-3 bg-blue-50 border border-blue-200 rounded-lg">
                                            ...
                                            <label htmlFor="includeMSE" className="flex-1 text-sm">
                                                <span className="font-semibold text-blue-900">🔬 Include Mental Status Exam (MSE) Analysis</span>
                                                <p className="text-blue-700 mt-1">
                                                    AI will analyze video for appearance, behavior, affect, and speech observations. Takes slightly longer to process. (Video only)
                                                </p>
                                            </label>
                                        </div>
                                    </>
